  ...
  <div className="flex flex-col items-center justify-center py-28 gap-5">
    <div className="relative">
      <div className="w-24 h-24 rounded-full bg-purple-50 flex items-center justify-center">
        <Headphones size={38} className="text-purple-400" />
      </div>
      <div className="absolute -right-1 -bottom-1 w-7 h-7 bg-purple-100 rounded-full flex items-center justify-center">
        <span className="text-purple-500 text-base font-black">?</span>
      </div>
    </div>
    <div className="text-center">
      <p className="text-gray-700 font-bold text-base">No listening tests yet</p>
      <p className="text-gray-400 text-sm mt-1">Complete your first test to see your progress here.</p>
    </div>
    <button
      onClick={onStart}
      className="mt-1 px-6 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-bold rounded-xl shadow transition"
    >
...
const ListeningHistoryPage = () => {
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, #faf5ff 0%, #f5f3ff 40%, #f0fdf4 100%)',
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-900 leading-tight">Listening History</h1>
              <p className="text-xs text-gray-400 mt-0.5">Track your IELTS listening progress</p>
            </div>
          </div>
          <button
            onClick={() => navigate('/listening')}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-gray-600 hover:text-purple-600 hover:border-purple-200 hover:bg-purple-50 transition shadow-sm"
          >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-3 p-4 border-b border-gray-100 bg-gray-50/60">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-2 text-sm rounded-lg border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none transition bg-white"
                />
              </div>
          ...
          {loading ? (
            ...
            <EmptyState onStart={() => navigate('/listening')} />
          ) : filtered.length === 0 ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filtered.map((item, i) => (
                  <tr
                    key={item.id}
                    onClick={() => navigate(`/listening/result/${item.id}`)}
                    className="group border-b border-gray-50 last:border-none hover:bg-purple-50/50 cursor-pointer transition-colors"
                    style={{ animationDelay: `${i * 40}ms` }}
                  >
                    <td className="px-6 py-4">
                      <div className="font-bold text-gray-800 text-sm group-hover:text-purple-700 transition-colors leading-tight">
                        {item.test?.title || item.test_title || `Test #${item.test_id}`}
                      </div>
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-[10px] text-gray-400">#{item.id}</span>
                        <span className="inline-block bg-purple-50 text-purple-500 border border-purple-100 px-1.5 py-0.5 rounded-md text-[9px] font-bold tracking-wide uppercase">
                          IELTS Listening
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 text-right">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center bg-gray-50 group-hover:bg-purple-100 transition ml-auto">
