...
import { useExamHistory } from '../../hooks/exam/useExamHistory';
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            <div>
                <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
                    <Award className="text-indigo-600" size={32} /> Exam History
                </h1>
...
const HistoryCard = ({ submission }) => {
    ...
    // Phân tích trạng thái bài thi thực tế
    const isCompleted = submission.status === 'COMPLETED';
    
    // ✅ CẬP NHẬT LOGIC: Kiểm tra score === null thay vì score === 0 để xác định xem đã có kết quả chưa
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-all group flex flex-col md:flex-row gap-6 items-center">
                ...
                <div className="mt-2">
                    {isInProgress ? (
                        <span className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-600 px-2.5 py-1 rounded-md text-[10px] font-bold border border-amber-200 uppercase tracking-wide">
                            <Clock size={12}/> In Progress ({submission.current_step})
                        </span>
                    ) : isGrading ? (
                        ...
                        <span className="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-600 px-2.5 py-1 rounded-md text-[10px] font-bold border border-emerald-200 uppercase tracking-wide">
                            <CheckCircleIcon /> Graded
                        </span>
                    )}
                </div>
            </div>

            {/* 2. SKILL SCORES (MIDDLE) */}
            ...
            <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                <div className="flex flex-col items-center min-w-[60px]">
                    ...
                    {/* ✅ CẬP NHẬT LOGIC HIỂN THỊ OVERALL: Nếu đang chấm dở thì hiện "?", nếu đã chấm xong thì hiện điểm (chấp nhận 0.0) */}
                    <span className={`text-3xl font-black ${(isGrading || isInProgress) ? 'text-slate-300' : 'text-indigo-600'}`}>
                        {(isGrading || isInProgress || submission.overall_score === null) ? "?" : Number(submission.overall_score).toFixed(1)}
                    </span>
                </div>
                ...
                >
                    {isInProgress ? <ArrowRight size={20} /> : <ChevronRight size={20} />}
                </Link>
            </div>
...
const ScoreBadge = ({ icon: Icon, label, score, color }) => {
    // ✅ CẬP NHẬT LOGIC SCORE >= 0: Xử lý an toàn để hiển thị số 0
    ...
    return (
        <div className={`flex flex-col items-center p-2 rounded-xl border w-[72px] transition-all ${colors[color]}`}>
            <div className={`flex items-center gap-1 mb-1 ${hasScore ? 'opacity-80' : 'opacity-50'}`}>
                <Icon size={12} /> <span className="text-[10px] font-bold">{label}</span>
            </div>
            
            {/* Hiển thị điểm đã được format (VD: 0.0) */}
