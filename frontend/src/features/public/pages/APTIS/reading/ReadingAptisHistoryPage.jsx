...
import { 
  ...
  Card, Empty, Skeleton, Breadcrumb 
} from 'antd';
import { 
  ArrowLeftOutlined, 
  EyeOutlined, 
...
// Function to select Tag color for CEFR
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          ...
          onClick={() => navigate(`/aptis/reading/result/${record.test_id}`)}
        >
          View Details
        </Button>
      ),
    ...
    <Layout style={{ minHeight: '100vh', backgroundColor: '#f8fafc' }}>
      ...
      <Content style={{ padding: '40px 24px', maxWidth: 1000, margin: '0 auto', width: '100%' }}>
        ...
        <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          {/* Mini Stats (Total tests & Highest score) */}
          {history.length > 0 && (
            <Card className="rounded-2xl border-orange-100 bg-orange-50/50 shadow-sm" styles={{ body: { padding: '12px 24px' } }}>
              <div className="flex items-center gap-6">
                <div className="text-center">
                  ...
                  <Text className="text-xl font-black text-orange-800">{history.length}</Text>
                </div>
