...
import { ArrowLeft, History, Calendar, Clock, CheckCircle2, Mic, ExternalLink, Award } from 'lucide-react';
...
const getCefrColorStyle = (level) => {
  ...
  if (level?.includes('B')) return { bg: 'bg-rose-100', text: 'text-rose-700', border: 'border-rose-200' };
    ...
    <Layout style={{ minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: "'Inter', sans-serif" }}>
      <Content style={{ padding: '40px 24px', maxWidth: 1040, margin: '0 auto', width: '100%' }}>
        ...
        <button
          ...
          className="group flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-full text-slate-600 font-semibold mb-8 hover:bg-slate-50 hover:text-rose-600 hover:border-rose-200 transition-all shadow-sm"
        >
        ...
        {/* BANNER */}
        <div className="mb-10 bg-white p-8 rounded-3xl border border-rose-50 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-rose-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 opacity-60 pointer-events-none" />
          
          <div className="flex items-center gap-5 relative z-10">
            <div className="p-4 bg-gradient-to-br from-rose-500 to-pink-500 text-white rounded-2xl shadow-lg shadow-rose-200">
          ...
          {stats && (
            <div className="flex bg-slate-50 rounded-2xl p-4 border border-slate-100 relative z-10">
              ...
              <div className="px-6 text-center">
                ...
                <Text className="text-2xl font-black text-rose-600">{stats.gradedSubmissions || 0}</Text>
              </div>
            </div>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm text-center py-24 flex flex-col items-center">
            ...
            <p className="text-slate-500 mb-8 max-w-md">You haven't taken any Speaking tests yet. Start practicing to see your progress here.</p>
            <button
              onClick={handleGoBack}
              className="bg-rose-600 hover:bg-rose-700 text-white font-bold h-12 px-8 rounded-xl shadow-md shadow-rose-200 transition-all hover:-translate-y-0.5"
            >
              Start Practice
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {history.map((record) => {
              ...
              return (
                <div key={record.id} className="group bg-white rounded-3xl border border-slate-100 p-6 flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-rose-100 transition-all duration-300 relative overflow-hidden">
                  
                  {/* Card Header */}
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        {isGraded ? <CheckCircle2 size={16} className="text-rose-500" /> : <Clock size={16} className="text-amber-500" />}
                  ...
                  <div className="bg-slate-50/80 rounded-2xl p-4 mb-6 flex items-center justify-between border border-slate-100/50">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-100 ${isGraded ? 'text-rose-500' : 'text-slate-300'}`}>
                        <Award size={20} />
                      </div>
                      <div>
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-slate-50 hover:bg-rose-600 text-slate-600 hover:text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all duration-300"
                  >
