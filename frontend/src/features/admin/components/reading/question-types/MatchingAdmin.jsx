...
const MatchingAdmin = ({ question, onChange }) => {
  ...
  const handleUpdateOption = (oldKey, newKey, newValue) => {
    ...
    // If the key name changes (e.g. i -> v)
    if (oldKey !== newKey) {
      delete newOptions[oldKey];

      // If the old key was the correct answer, update it to the new key
  ...
  const handleDeleteOption = (key) => {
    ...
    // Reset if the deleted option was the correct answer
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-1">
          ...
          {optionsArray.length === 0 && (
            <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded text-gray-400 text-sm">
              No options yet.<br />Click “Bulk Input” or “Add row”.
            </div>
          )}
        </div>
      )}
