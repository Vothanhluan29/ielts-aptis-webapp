...
export const useReadingAptisEdit = () => {
              ...
              const mappedQs = (group.questions || []).map((q) => {
                ...
                if (q.question_type === 'MULTIPLE_CHOICE') {
                  if (
                    ...
                    const foundIdx = optionsArray.findIndex(
                  ...
                  if (
                    ...
                    correctIndex = parts.map((l) =>
                      letters.indexOf(l.toUpperCase()).toString()
                    );
                  } else if (
                    typeof q.correct_answer === 'string' &&
                  ...
                  q.question_type === 'MATCHING_OPINIONS' ||
  ...
  const onFinishFailed = (errorInfo) => {
    ...
    message.error(
      'Please fill in all required fields (check closed parts).'
    );
  };
  ...
  const onFinish = async (values) => {
    ...
    if (partsCount > 5) {
      message.error(
        `Limit exceeded: Reading test can contain a maximum of 5 parts (Current: ${partsCount}).`
      );
      return;
    }

    const totalQuestions =
      values.parts?.reduce((total, part) => {
        return total + (part.questions?.length || 0);
      }, 0) || 0;

    if (totalQuestions > 25) {
      message.error(
        `Limit exceeded: Reading test can contain a maximum of 25 questions (Current: ${totalQuestions}).`
      );
      return;
    }
                ...
                if (q.question_type === 'REORDER_SENTENCES') {
                  ...
                  if (/^[0-9,\s]+$/.test(ans)) {
                    ...
                    exactCorrectText = ans
                      .split(',')
                      .map((idx) => letters[Number(idx.trim())])
                      .join('-');
                  } else {
                    exactCorrectText = ans
                      .toUpperCase()
                  ...
                  exactCorrectText =
                    q.options[Number(q.correct_answer)]?.trim() || '';
